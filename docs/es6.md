...
执行上下文是一个对象，包括三个属性：变量对象，作用域链，this 指针。它影响着变量解析，变量作用域和函数的 this 指向。js 执行的时候会维护一个上下文栈。
全局上下文，函数上下文。

- 变量对象：argument 函数 函数内的变量

- 作用域链:当查找变量的时候，会在当前上下文的变量对象中查找，如果没有查找到，就会在那父级的上下文的变量对象中查找，一直到全局上下文的对象变量。这样由多个执行上下文的变量 对象构成的链表叫做作用域链。
...
1.  函数表达式和函数声明
    ...
    函数声明会被提升到作用域的最前面，(变量提升)（解析和运行）

    函数表达式创建的函数是在运行时进行赋值,且要等到表达式赋值完成后才能调用

2.  IIFE(立即执行函数表达式)
    ((function f(){f=1;console.log(f);})())//扩展了作用域，实现作用域隔离，避免了内存泄漏，可以减少闭包占用内存
...
3.  匿名函数与闭包
    ...
    闭包：闭包是指有权访问另一个函数作用域中的变量的函数。方式： 一个函数内部创建另一个函数，通过另一个函数访问这个函数的局部变量。
    作用：局部变量驻留在内存中，可以避免使 用全局变量。
    缺点：闭包里作用域返回的局部变量资源不会被立刻销毁回收，所以可能会占用更 多的内存

4.  具名函数(NFE) 在该函数内部是 const。无法重新赋值

    (function f(){f=1;console.log(f);})()//functon
    (function f(){c=1;console.log(c);})()//1
